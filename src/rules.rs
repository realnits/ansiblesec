...
use crate::errors::{Result, AnsibleSecError};
...
#[serde(tag = "type")]
pub enum RuleType {
    DisallowModule { modules: Vec<String> },
    RequireVault { exceptions: Vec<String> },
    DisallowHardcodedCredentials,
    RequireNoLogForSensitive,
    CheckPermissions { max_permissions: String },
    CustomYamlPath { path: String, expected_value: Option<String> },
}
...
impl RulesEngine {
    ...
    pub fn validate(&self) -> Result<()> {
        for rule in &self.rules {
            ...
            if rule.id.is_empty() {
                return Err(AnsibleSecError::RuleValidationError("Rule has empty ID".to_string()));
            }
            if rule.name.is_empty() {
                return Err(AnsibleSecError::RuleValidationError(format!("Rule {} has empty name", rule.id)));
            }
            
            // Validate severity
            match rule.severity.to_uppercase().as_str() {
                "CRITICAL" | "HIGH" | "MEDIUM" | "LOW" | "INFO" => {}
                _ => return Err(AnsibleSecError::RuleValidationError(format!("Rule {} has invalid severity: {}", rule.id, rule.severity))),
            }
        }
    ...
    pub fn list_rules(&self) {
        ...
        for rule in &self.rules {
            ...
            println!("[{}] {} - {} ({})", status, rule.id, rule.name, rule.severity);
    ...
    fn default_rules() -> Vec<Rule> {
        vec![
            Rule {
                ...
                description: "Prevents use of risky modules like shell, command, and raw".to_string(),
                severity: "HIGH".to_string(),
                enabled: true,
                rule_type: RuleType::DisallowModule {
                    modules: vec!["shell".to_string(), "command".to_string(), "raw".to_string()],
                },
            },
            Rule {
                id: "POLICY_002".to_string(),
                name: "Require Ansible Vault".to_string(),
                description: "Ensures sensitive variables are encrypted with Ansible Vault".to_string(),
            ...
            Rule {
                ...
                description: "Prevents hardcoded passwords and credentials in playbooks".to_string(),
                severity: "CRITICAL".to_string(),
                enabled: true,
            ...
            Rule {
                ...
                description: "Validates file/directory permissions are not overly permissive".to_string(),
