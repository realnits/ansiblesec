...
impl Scanner {
    pub fn new(config: Config, threads: usize, enable_cache: bool) -> Self {
        ...
        let secret_detector = if let Some(ref rules_file) = config.secrets.rules_file {
            SecretDetector::from_file(rules_file, config.secrets.entropy_threshold)
                .unwrap_or_else(|e| {
                    eprintln!("Warning: Failed to load secrets rules from {}: {}", rules_file, e);
    ...
    pub fn scan(&self, path: &Path) -> Result<ScanFindings> {
        ...
        eprintln!("📂 Found {} Ansible file{} to scan", 
            files.len(), 
        ...
        let results: Vec<_> = files
            ...
            .filter_map(|(idx, file)| {
                ...
                if files.len() > 1 {
                    ...
                    eprint!("\r⏳ Progress: [{:3.0}%] Scanning file {}/{}...", 
                        progress, idx + 1, files.len());
                }
        ...
        if files.len() > 1 {
            eprintln!("\r✅ Scan complete! Analyzed {} files.                    ", files.len());
        } else {
    ...
    fn is_excluded(&self, path: &Path) -> bool {
        ...
        self.config.general.exclude_paths.iter().any(|exclude| {
    ...
    fn aggregate_findings(&self, results: Vec<FileFinding>, files_scanned: usize) -> ScanFindings {
        ...
        for file_finding in results {
            ...
            if has_findings {
                if file_finding.findings.iter().any(|f| f.rule_id.starts_with("SECRET")) {
                    secrets.push(file_finding.clone());
                }
                if file_finding.findings.iter().any(|f| f.rule_id.starts_with("POLICY")) {
                    policy_violations.push(file_finding.clone());
                }
                if file_finding.findings.iter().any(|f| f.rule_id.starts_with("LINT")) {
...
impl ScanFindings {
    ...
    pub fn total_findings(&self) -> usize {
        self.summary.critical + self.summary.high + self.summary.medium + 
        self.summary.low + self.summary.info
    }
}
